...
def _render_leaderboard(test_results, best_name: str, config: Dict[str, Any]) -> None:
    ...
    with panel('All candidates', 'Test split, best value per column highlighted'):
...
def _render_curves(all_models, X_test, y_test) -> None:
    ...
    with left:
        with panel('ROC', 'True positives against false positives'):
            fig = go.Figure()
            fig.add_trace(
                go.Scatter(
    ...
    with right:
        with panel('Precision-recall', 'The honest view under imbalance'):
...
def _render_calibration(all_models, calibrated_model, best_name, X_test, y_test, load_results_fn) -> None:
    ...
    with panel('Reliability diagram', 'Closer to the diagonal means the probabilities can be trusted'):
...
def _render_cost(all_models, calibrated_model, best_name, X_test, y_test, config) -> None:
    ...
    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
    with panel('Cost by model', 'Over the {len(y_test):,} assets in the test split'):
        st.dataframe(
    ...
    # ------------------------------------------------- threshold sweep
    with panel('Where to set the alert threshold', 'Total cost against the flagging threshold'):
